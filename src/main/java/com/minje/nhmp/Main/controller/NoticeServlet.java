...
import com.minje.nhmp.Main.service.NoticeService;
...
public class NoticeServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if(request.getServletPath().equals("/mainnoticeinsert")) {
			// 공지사항 추가 컨트롤러
			...
			int result = new NoticeService().insertNotice(c);
		...
		if(request.getServletPath().equals("/mainnoticelist")) {
			...
			// 전체 공지글 조회 컨트롤러
			ArrayList<NoticeVo> list = notice.selectList(startRow, endRow);
		...
		if(request.getServletPath().equals("/mainnoticedetail")) {
			// 공지글 상세보기 처리용 컨트롤러
			int noticeno = Integer.parseInt(request.getParameter("no"));
			NoticeVo notice = new NoticeService().selectOne(noticeno);
		...
		if(request.getServletPath().equals("/mainnoticeupdate")) {
			// 공지사항 삭제 컨트롤러
