...
import com.minje.nhmp.Main.service.QnaService;
...
public class QnaServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if(request.getServletPath().equals("/mainqnainsert")) {
			// Qna 등록 코드
			request.setCharacterEncoding("UTF-8"); // 전송온 값에 한글이 있다면, 인코딩 처리
		
			QnaVo qnaVoInsert = new QnaVo();
		...
		if(request.getServletPath().equals("/mainqnalist")) {
			// Qna 페이징 코드
			int currentPage = 1;
			...
			int maxPage = listCount / limit ; //총 페이지 수 계산
		...
		if(request.getServletPath().equals("/mainqnadetail")) {
			// Qna 디테일 코드
			int qnaNoDetail = Integer.parseInt(request.getParameter("no"));
	
			ArrayList<CommentVo> listDetail = new CommentService().selectList(qnaNoDetail);
		...
		if(request.getServletPath().equals("/mainqnadelete")) {
			// Qna 삭제용 코드
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(request.getServletPath().equals("/mainqnaupdate")) {
			// Qna 수정 코드
			request.setCharacterEncoding("utf-8");
